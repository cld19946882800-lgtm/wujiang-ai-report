...
# ==========================================
# 🚀 核心大模型双引擎配置 (已接入云端保险箱防盗)
# ==========================================
# 从 Streamlit Secrets 自动安全读取 Key
...
def generate_visual_pdf(df_summary, top10_applicants, applicant_types, loss_reasons):
    ...
    ax3 = axes[1, 0]
    ax3.pie(applicant_types.values, labels=applicant_types.index, autopct='%1.1f%%', startangle=140, colors=plt.cm.Pastel1.colors)
...
with col_right:
    ...
    if st.session_state.started:
        if not st.session_state.processed:
            with st.status("智能体全链路运作中...", expanded=True) as status:
                ...
                try:
                    ...
                    prompt = f"""
                    你是一位吴江区市场监管局的高级分析师。请根据以下我算出的真实底层数据，写一段高质量的《知识产权月度简报》正文。
                    新增授权 {totals['授权总量']} 件，有效总盘达 {totals['有效总量']} 件。
                    【{', '.join(top3_dist)}】包揽最多授权量。
                    火力最猛企业：“{top10_app.index[0]}”（贡献{top10_app.values[0]}件）。
                    核心技术突破在 IPC 分类号【{', '.join(top5_ipc.index)}】。
                    要求：用“政府智库”口吻，分两个自然段，300-400字。
                    """
        ...
        if st.session_state.processed:
            ...
            with btn_col1:
                st.download_button("📝 下载研判简报", data=st.session_state.word_file, file_name="吴江区知识产权_本期研判.docx", use_container_width=True)
            with btn_col2:
                st.download_button("📊 下载多板块底稿", data=st.session_state.excel_file, file_name="吴江区数据分析包.xlsx", use_container_width=True)
            with btn_col3:
                st.download_button("📈 下载大屏(PDF)", data=st.session_state.pdf_file, file_name="吴江区专利大屏.pdf", use_container_width=True)
